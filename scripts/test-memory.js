...
async function flush() {
  // Wait for the in-process queue to drain. Real Hyperspell writes take 1-2s each
  // so 18 writes serialized at concurrency 2 can take 30-40s. Mock returns instantly.
  const max = hs.isMock ? 60 : 1200;             // up to 60s real
  const tickMs = hs.isMock ? 50 : 100;
  for (let i = 0; i < max; i++) {
    const s = queue.stats();
    if (s.depth === 0 && s.active === 0) return;
    if (i % 50 === 0 && i > 0) info(`  flushing… queue: ${JSON.stringify(s)}`);
    await new Promise((r) => setTimeout(r, tickMs));
  }
  warn(`flush timeout · queue: ${JSON.stringify(queue.stats())}`);
}

async function waitForIndexing(seconds) {
  if (hs.isMock) return;
  info(`waiting ${seconds}s for Hyperspell to index documents…`);
...
async function main() {
  ...
  // Real Hyperspell needs time to index documents before they show up in /memories/query.
